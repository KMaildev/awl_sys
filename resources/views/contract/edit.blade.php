<form action="{{ route('contract.update', $contract_edit->id) }}" method="post" id="create-form" autocomplete="off">
    @csrf
    @method('PUT')
    ...
    <tr>
        ...
        <td>
            <select class="form-select form-select-sm @error('demand_id') is-invalid @enderror" name="demand_id">
                ...
                @foreach ($demands as $demand)
                    <option value="{{ $demand->id }}" @if ($demand->id == $contract_edit->demand_id) selected @endif>
        ...
        <td>
            <input type="text" class="form-control form-control-sm @error('remark') is-invalid @enderror"
                name="remark" value="{{ $contract_edit->remark ?? '' }}">
        </td>
